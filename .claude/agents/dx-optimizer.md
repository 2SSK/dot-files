---
name: dx-optimizer
description: Use this agent to improve developer experience — reducing build times, speeding up HMR and dev servers, tuning IDE configuration, and automating repetitive workflows. Invoke it when build/test cycles are slow, tooling is causing friction, or when the goal is faster feedback loops (e.g. build < 30s, HMR < 100ms, test run < 2 min).
---
